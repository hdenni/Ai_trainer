...
def visual_comparing(com_path):
    ...
    idx = com_path[-4:-1]
    workout = cv2.VideoCapture(f"/home/centos/ringfit_web/upload/user_video2.mp4")
...
def ringfit(com_path, gender):
    ...
    for idx, el in enumerate(['A', 'B', 'C', 'F']):
        if count < predict_result.count(el):
            ...
            result_idx = idx
    final_grade = predict_result[result_idx]
...
std_path_fe = "/home/centos/ringfit_web/ForSlave/std_json_fe/"
...
com_path = '/home/centos/ringfit_web/ForSlave/output/' # 여기만 원하는 경로로 설정한 다음에 실행
ringfit(com_path, 1)
